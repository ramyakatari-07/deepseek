    ...
    return(
        <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand?'p-4 w-64':'md:w-20 w-0 max-md:overflow-hidden'}`}>
          ...
          className={`flex ${expand? "flex-row gap-10":"flex-col items-center gap-8"}`}>
            ...
            <div className='group relative flex items-center justify-center hover:bg-grey-500/20 transition-all duration-300 h-9 w-9 aspect-square rounded-lg cursor-pointer'>
          ...
          <button className={`mt-8 flex items-center justify-center cursor-pointer ${expand ? "bg-primary hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max" : "group relative h-9 w-9 mx-auto hover:bg-grey-500/30 rounded-lg"}`}>
...
>
  {isSignedIn ? (
    ...
    <Image src={assets.profile_icon} alt="Sign In" className="w-7" />
  )}
  {expand && <span>{isSignedIn ? "My Profile" : "Sign In"}</span>}
</div>
